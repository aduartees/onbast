...
import { PRICING_PLANS_QUERY, PRICING_ADDONS_QUERY, SETTINGS_QUERY } from "@/sanity/lib/queries";
...
export default async function PricingPage({
  ...
  const [settings, plans, addons] = await Promise.all([
    ...
    client.fetch(PRICING_ADDONS_QUERY, {}, { next: { revalidate } }),
  ]);
            ...
            <FadeIn delay={0.1}>
              <article className="relative rounded-3xl border border-white/10 bg-neutral-900/10 p-4 md:p-10 shadow-2xl overflow-hidden">
                ...
                <div className="relative">
                  <PricingWizard plans={plans} addons={addons} initialPlanId={planId} initialLocation={loc} />
                </div>
              </article>
            </FadeIn>
          </div>
