...
import { HOME_PAGE_QUERY, HOME_SERVICES_SCHEMA_QUERY, SETTINGS_QUERY } from "@/sanity/lib/queries";
import { generateHomeOrganizationServicesSchema } from "@/lib/seo";

export const dynamic = 'force-dynamic';

export default async function Home() {
  const [homeData, services, siteSettings] = await Promise.all([
    client.fetch(HOME_PAGE_QUERY),
    client.fetch(HOME_SERVICES_SCHEMA_QUERY),
    client.fetch(SETTINGS_QUERY),
  ]);

  const jsonLd = generateHomeOrganizationServicesSchema({ siteSettings }, services);
